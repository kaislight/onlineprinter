...
#读取数据
print('正在读取数据·····')
data = pd.read_csv('./static/food/code/fooddata/meal_order_detail.csv', usecols=['dishes_name', 'order_id', 'emp_id'])
data1 = pd.read_csv('./static/food/code/fooddata/meal_order_info.csv', usecols=['order_status', 'info_id'])
#删除\r\n字符
print('正在删除字符·····')
data = data.replace('\r', '', regex=True).replace('\n', '', regex=True)
###数据预处理
#去除'白饭/小碗'的数据
print("正在去除'白饭/小碗'的数据·····")
data2 = data.drop(data.loc[(data['dishes_name'] == '白饭/小碗')].index, axis=0)
#去除'白饭/大碗'的数据
print("正在去除'白饭/大碗'的数据·····")
data2.drop(data2.loc[(data2['dishes_name'] == '白饭/大碗')].index, inplace=True, axis=0)

#寻找订单状态不为1的索引
print("正在寻找订单状态不为1的索引·····")
b = data1.loc[(data1['order_status'] != 1)].index
#寻找订单状态不为1的订单ID
print("正在寻找订单状态不为1的订单ID·····")
b1 = data1['info_id'][b]
#删除订单状态不为1的数据
print("正在删除订单状态不为1的数据·····")
for i in b1:
    a = data2.drop(data2.loc[(data2['order_id'] == i)].index, axis=0)

##删除点餐数量小于3的用户
print("正在删除点餐数量小于3的用户·····")
n1 = {}   #创建用户ID与点餐数量的字典
for i in a['emp_id']:
    if i not in n1:
        n1[i] = 1
...
print("正在对客户所点的每一道菜,推荐与其相似度最高的一道菜，用aa表示·····")
aa = {}
for i in cs:
    aa[i] = []
    for j in cs[i]:
